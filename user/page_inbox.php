    ...
    <div class="flex-1 min-w-0 p-4 md:p-8">
        ...
        <?php else: ?>
            ...
            <div
                ...
                <div
                    class="p-6 border-b border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4 bg-white/5 backdrop-blur-xl">
                    <h2 class="text-xl font-bold flex items-center gap-3 text-white">
                        <span
                            class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center shadow-lg shadow-indigo-500/20">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
                        ...
                        <form action="create_campaign.php" method="POST" id="campaign-form">
                            <input type="hidden" name="page_id" value="<?php echo $page['id']; ?>">
                            <button type="submit"
                                class="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-6 py-3 rounded-xl shadow-lg shadow-indigo-600/20 transition-all transform hover:-translate-y-0.5 disabled:opacity-20 disabled:grayscale-[0.5] disabled:translate-y-0 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                ...
                                <span><?php echo __('create_campaign'); ?></span>
                ...
                <div class="overflow-y-auto max-h-[600px] custom-scrollbar bg-[#0f172a]/50">
                    <table class="w-full text-start border-collapse" id="leads-table">
                        ...
                        <tbody class="divide-y divide-gray-800/50" id="leads-tbody">
                            <?php
                            $stmt = $pdo->prepare("SELECT * FROM fb_leads WHERE page_id = ? ORDER BY created_at DESC LIMIT 500");
                            $stmt->execute([$page['id']]);
                                        ...
                                        <td class="px-6 py-4 text-start">
                                            <div class="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="text-[10px] text-gray-500"><?php echo __('fb_user_role'); ?></div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-start">
                                ...
                                <tr id="no-leads-row">
                                    <td colspan="4">
                                        <div class="flex flex-col items-center justify-center py-24 text-gray-500">
                                            <div
                                                class="w-20 h-20 bg-gradient-to-br from-gray-800 to-gray-900 rounded-full flex items-center justify-center mb-6 shadow-inner shadow-black/50">
                                            ...
                                            <p class="text-lg font-medium text-gray-400"><?php echo __('no_leads_found'); ?></p>
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
...
<div id="token-modal"
    ...
    <div
        ...
        <h3 class="text-2xl font-bold text-white text-center mb-3 text-red-500"><?php echo __('token_expired_title'); ?>
        </h3>
...
<?php if (!$showSelector): ?>
    <script>
        ...
        window.filterLeads = function () {
            ...
            const rows = document.querySelectorAll('#leads-tbody tr:not(#no-leads-row)');

            rows.forEach(row => {
                const nameCell = row.querySelector('.font-bold.text-white');
                const idCell = row.querySelector('.font-mono');

                if (nameCell || idCell) {
                    const name = nameCell ? nameCell.textContent.toLowerCase() : '';
                    const psid = idCell ? idCell.textContent.toLowerCase() : '';

                    if (name.includes(query) || psid.includes(query)) {
                        row.style.display = '';
                    } else {
