 ...
 * Version: 018
 * Description: Ensures that wa_campaigns and settings tables have the latest schema columns.
 */

function migrate_018_up($pdo)
{
    // 1. Ensure wa_campaigns has 'gateway_mode'
    // We assume wa_campaigns exists (from migration 017), but we force check columns

    // Using try-catch only for the specific ALTER commands to avoid stopping if column exists (and IF NOT EXISTS syntax isn't fully supported for columns in all MySQL versions with simple ALTER)
    // But here we check rowCount first.
    ...
    // 2. Ensure user_wa_settings exists
    ...
    // Check for 'external_config' column
