                ...
                if (!empty($tableData['data'])) {
                    foreach ($tableData['data'] as $row) {
                        ...
                        $values = array_map(function ($value) use ($pdo) {
                            return $value === null ? 'NULL' : $pdo->quote($value); }, array_values($row));
...
<div id="progressModal" class="progress-container">
    <div class="progress-box">
        ...
        <p class="text-gray-300 text-sm mb-4" id="progressMessage"><?php echo $lang === 'ar' ? 'يرجى الانتظار...' : 'Please wait...'; ?></p>
    ...
    <div class="modal-content">
        <div class="text-center">
            ...
            <div class="mt-2 text-sm text-gray-400">
                <p><?php echo $lang === 'ar' ? 'هل أنت متأكد أنك تريد حذف هذه النسخة الاحتياطية؟ لا يمكن التراجع عن هذا الإجراء.' : 'Are you sure you want to delete this backup? This action cannot be undone.'; ?></p>
                <p class="mt-2 text-indigo-400 font-mono text-xs" id="deleteFileName"></p>
            </div>
        </div>
        <div class="mt-6 flex justify-center gap-3">
            <button type="button" onclick="closeDeleteModal()" class="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition-colors">
                <?php echo $lang === 'ar' ? 'إلغاء' : 'Cancel'; ?>
            </button>
            <form method="POST" class="inline">
                <input type="hidden" name="delete_backup_name" id="deleteInputName" value="">
                <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition-colors font-bold shadow-lg">
...
<div class="flex min-h-screen pt-4">
    ...
    <div class="flex-1 min-w-0 p-4 md:p-8">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-white mb-2"><?php echo __('backup_restore'); ?></h1>
            <p class="text-gray-400"><?php echo __('backup_restore_desc'); ?></p>
        </div>
        ...
        <?php if ($error): ?>
                <div class="bg-red-500/10 border border-red-500/50 text-red-400 px-6 py-4 rounded-xl mb-6 flex items-center gap-3">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div>
                    <div class="flex items-center gap-3 mb-4">
                        <div class="p-3 bg-indigo-500/20 rounded-xl">
                            <svg class="w-6 h-6 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"></path></svg>
                        </div>
                        <h2 class="text-xl font-bold text-white"><?php echo __('create_backup'); ?></h2>
                    </div>
                    <p class="text-gray-400 mb-6 leading-relaxed"><?php echo __('backup_includes'); ?> (JSON)</p>
                </div>
                
                <div class="mt-auto">
                    <form method="POST" id="createBackupForm">
                        <input type="hidden" name="create_backup" value="1">
                        <button type="submit" class="w-full bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white font-bold py-3.5 px-6 rounded-xl transition-all shadow-lg transform hover:-translate-y-0.5 flex items-center justify-center gap-2">
            ...
            <div class="glass-card rounded-2xl p-6 flex flex-col h-full">
                <div>
                    <div class="flex items-center gap-3 mb-4">
                        <div class="p-3 bg-purple-500/20 rounded-xl">
                            <svg class="w-6 h-6 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                        </div>
                        <h2 class="text-xl font-bold text-white"><?php echo __('restore_backup'); ?></h2>
                    </div>
                ...
                <div class="mt-auto">
                    <form method="POST" enctype="multipart/form-data" onsubmit="return confirm('<?php echo __('confirm_restore'); ?>');">
                        <div class="flex gap-3">
                            <div class="relative flex-1">
                                <input type="file" name="backup_file" accept=".json" required class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" onchange="document.getElementById('fileNameDisplay').textContent = this.files[0].name">
                                <div class="w-full bg-gray-800 border border-gray-700 text-gray-400 rounded-xl px-4 py-3.5 flex items-center justify-between">
                                    <span id="fileNameDisplay" class="truncate max-w-[150px]"><?php echo $lang === 'ar' ? 'اختر ملف...' : 'Choose file...'; ?></span>
                            ...
                            <button type="submit" name="restore_backup" class="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-bold py-3.5 px-6 rounded-xl transition-all shadow-lg transform hover:-translate-y-0.5 whitespace-nowrap">
        ...
        <div class="glass-card rounded-2xl p-6">
            <h2 class="text-xl font-bold text-white mb-6"><?php echo __('backup_list'); ?> (<?php echo count($backups); ?>)</h2>
            
            <?php if (empty($backups)): ?>
                    <div class="text-center py-12">
                        <svg class="w-16 h-16 mx-auto text-gray-600 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"></path></svg>
                        <p class="text-gray-500 text-lg"><?php echo __('no_backups'); ?></p>
                    </div>
            <?php else: ?>
                        ...
                        <table class="w-full text-left">
                            <thead>
                                <tr class="text-gray-400 border-b border-gray-700/50">
                                    <th class="pb-4 px-4"><?php echo __('backup_name'); ?></th>
                                    <th class="pb-4 px-4"><?php echo __('backup_size'); ?></th>
                                    <th class="pb-4 px-4"><?php echo __('backup_date'); ?></th>
                                    <th class="pb-4 px-4 text-center"><?php echo __('actions'); ?></th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-800">
                                <?php foreach ($backups as $backup): ?>
                                        <tr class="hover:bg-gray-800/30 transition-colors">
                                            <td class="py-4 px-4 font-medium text-white ltr:text-left rtl:text-right" dir="ltr">
                                                <?php echo htmlspecialchars($backup['name']); ?>
                                            </td>
                                            <td class="py-4 px-4 text-gray-400"><?php echo formatBytes($backup['size']); ?></td>
                                            <td class="py-4 px-4 text-gray-400"><?php echo date('Y-m-d H:i:s', $backup['date']); ?></td>
                                            <td class="py-4 px-4">
                                                <div class="flex items-center justify-center gap-2">
                                                    <a href="?download=<?php echo urlencode($backup['name']); ?>" class="bg-blue-600/20 text-blue-400 hover:bg-blue-600/30 px-3 py-2 rounded-lg text-sm transition-colors flex items-center gap-1">
                                                    ...
                                                    <button type="button" 
                                                            ...
                                                            class="bg-red-600/20 text-red-400 hover:bg-red-600/30 px-3 py-2 rounded-lg text-sm transition-colors flex items-center gap-1 cursor-pointer">
