...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'check_updates') {
        ...
        foreach ($commands as $cmd) {
            ...
            foreach ($output as $line) {
                if (strpos($line, 'error: No such remote') !== false) continue;
                 $newLog[] = ['type' => 'info', 'message' => $line];
            }
        }
...
<div class="flex min-h-screen pt-4">
    ...
    <div class="flex-1 min-w-0 p-4 md:p-8">
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-gray-400">
                    ...
                    <span class="text-green-400 font-mono bg-green-500/10 px-2 py-0.5 rounded-lg border border-green-500/20"><?php echo $currentVersion; ?></span>
                </p>
            </div>
            
             <a href="backup.php" class="bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 px-4 py-2 rounded-xl transition-colors text-sm font-bold flex items-center gap-2 border border-indigo-500/20">
        ...
        <!-- Warning -->
        <div class="bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 px-6 py-4 rounded-2xl mb-8 flex items-start gap-4">
        ...
        <div class="mb-8">
            <details class="group">
                <summary class="flex items-center gap-2 cursor-pointer text-gray-400 hover:text-white transition-colors">
                    ...
                    <span class="font-bold text-sm bg-gray-800/50 px-3 py-1 rounded-lg border border-white/5"><?php echo __('troubleshooting'); ?></span>
                </summary>
                <div class="glass-card bg-red-500/5 border border-red-500/10 rounded-2xl p-6 mt-4 animate-in slide-in-from-top-2">
                    <div class="flex items-center justify-between">
                        ...
                        <!-- Open Modal Button -->
                        <button type="button" onclick="document.getElementById('git-fix-modal').classList.remove('hidden')" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg font-bold text-xs shadow-lg transition-colors flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <form method="POST" class="contents">
                ...
                <button type="submit" class="glass-card p-6 rounded-2xl text-left hover:bg-white/5 transition-all group relative overflow-hidden w-full">
                    <div class="absolute inset-0 bg-blue-600/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
            ...
            <form method="POST" class="contents">
                ...
                <button type="submit" class="glass-card p-6 rounded-2xl text-left hover:bg-white/5 transition-all group relative overflow-hidden w-full">
                    <div class="absolute inset-0 bg-green-600/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                     <div class="relative z-10">
            ...
            <form method="POST" class="contents">
                ...
                <button type="submit" class="glass-card p-6 rounded-2xl text-left hover:bg-white/5 transition-all group relative overflow-hidden w-full">
                     <div class="absolute inset-0 bg-purple-600/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                     <div class="relative z-10">
        ...
        <div class="glass-card rounded-2xl p-6 border border-gray-800">
            ...
            <div class="bg-black/30 rounded-xl p-4 font-mono text-sm max-h-96 overflow-y-auto space-y-2 border border-black/20" dir="ltr"> <!-- Logs usually look better LTR -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-gray-800/50 rounded-xl p-4 border border-gray-700/50">
                    <div class="text-blue-400 font-bold mb-2 flex items-center gap-2">
                        <span class="w-6 h-6 rounded-full bg-blue-500/20 flex items-center justify-center text-xs">1</span>
                ...
                <div class="bg-gray-800/50 rounded-xl p-4 border border-gray-700/50">
                    <div class="text-green-400 font-bold mb-2 flex items-center gap-2">
                        <span class="w-6 h-6 rounded-full bg-green-500/20 flex items-center justify-center text-xs">2</span>
...
<!-- WARNING MODAL -->
<div id="git-fix-modal" class="fixed inset-0 z-[100] hidden bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-gray-900 border border-red-500/30 w-full max-w-lg rounded-3xl p-8 shadow-2xl animate-in fade-in zoom-in duration-300">
        <div class="w-20 h-20 rounded-full bg-red-500/10 flex items-center justify-center text-red-500 mx-auto mb-6 ring-4 ring-red-500/20">
        ...
        <div class="space-y-3">
             <a href="backup.php" class="w-full py-3.5 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl shadow-lg shadow-green-600/20 transition-all flex items-center justify-center gap-2 group">
            ...
            <form method="POST">
                ...
                <button type="submit" class="w-full py-3.5 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white font-bold rounded-xl border border-red-500/20 transition-all flex items-center justify-center gap-2">
                    <span><?php echo __('confirm_git_fix'); ?></span>
                </button>
            </form>
            
            <button onclick="document.getElementById('git-fix-modal').classList.add('hidden')" class="w-full text-gray-500 text-sm font-medium hover:text-white transition-colors py-2">
                <?php echo __('cancel'); ?>
            </button>
        </div>
    </div>
