<?php
/**
 * Migration: Create WhatsApp Campaigns Table
 * Version: 017
 * Description: Creates table for storing WhatsApp bulk send campaigns
 */

function migrate_017_up($pdo)
{
    $sql = "CREATE TABLE IF NOT EXISTS `wa_campaigns` (
        ...
        CONSTRAINT `fk_wa_campaigns_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
...
function migrate_017_down($pdo)
{
    $sql = "DROP TABLE IF EXISTS `wa_campaigns`;";
    $pdo->exec($sql);
    return true;
}

function migrate_017_description()
{
    return "Creates wa_campaigns table for WhatsApp bulk sending campaigns with support for multiple gateways and account rotation";
}
