...
<?php if (getSetting('social_whatsapp') && strpos($_SERVER['SCRIPT_NAME'], '/admin/') === false): ?>
    <?php
    $wa_link = getSetting('social_whatsapp');
    if (!preg_match("~^(?:f|ht)tps?://~i", $wa_link)) {
        $wa_link = "https://wa.me/" . preg_replace('/[^0-9]/', '', $wa_link);
    }
    ?>
    <a href="<?php echo $wa_link; ?>" target="_blank"
        class="fixed bottom-8 left-8 z-[100] w-14 h-14 rounded-full bg-[#25D366] text-white flex items-center justify-center shadow-2xl shadow-green-500/40 hover:scale-110 active:scale-95 transition-all duration-300">
