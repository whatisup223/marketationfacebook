...
<aside id="app-sidebar" x-data="{ 
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-1.5">
        ...
        if ($current_user):
            $avatar_path = !empty($current_user['avatar']) ? '../uploads/' . $current_user['avatar'] : '';
            ?>
            <a href="profile.php"
        ...
        <nav class="space-y-1" x-data="{ fbOpen: <?php echo $is_fb_open; ?>, waOpen: <?php echo $is_wa_open; ?> }">
            ...
            <div>
                <button @click="sidebarCollapsed ? (toggleSidebar(), fbOpen = true) : fbOpen = !fbOpen" 
                    ...
                    <div class="w-6 h-6 flex items-center justify-center shrink-0 transition-all duration-300" :class="sidebarCollapsed ? 'w-full' : 'w-6'">
                        <svg class="w-5 h-5 group-hover:text-indigo-400 transition-colors" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                        </svg>
                    </div>
                    <span class="ms-3 text-sm font-medium hide-on-collapse transition-all duration-300"><?php echo __('facebook'); ?></span>
                    <svg class="w-4 h-4 ms-auto transition-transform hide-on-collapse" :class="fbOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div :class="{ 'is-active': fbOpen && !sidebarCollapsed }"
                    ...
                    <a href="fb_accounts.php" title="<?php echo __('fb_accounts'); ?>" class="block py-1.5 text-xs <?php echo $current_page == 'fb_accounts.php' ? 'text-indigo-400 font-bold' : 'text-gray-500 hover:text-gray-300'; ?>"><?php echo __('fb_accounts'); ?></a>
                    <a href="page_inbox.php" title="<?php echo __('manage_messages'); ?>" class="block py-1.5 text-xs <?php echo $current_page == 'page_inbox.php' ? 'text-indigo-400 font-bold' : 'text-gray-500 hover:text-gray-300'; ?>"><?php echo __('manage_messages'); ?></a>
                    <a href="create_campaign.php" title="<?php echo __('setup_campaign'); ?>" class="block py-1.5 text-xs <?php echo $current_page == 'create_campaign.php' ? 'text-indigo-400 font-bold' : 'text-gray-500 hover:text-gray-300'; ?>"><?php echo __('setup_campaign'); ?></a>
                    <a href="page_auto_reply.php" title="<?php echo __('auto_reply'); ?>" class="block py-1.5 text-xs <?php echo $current_page == 'page_auto_reply.php' ? 'text-indigo-400 font-bold' : 'text-gray-500 hover:text-gray-300'; ?>"><?php echo __('auto_reply'); ?></a>
                    <a href="page_messenger_bot.php" title="<?php echo __('auto_reply_messages'); ?>" class="block py-1.5 text-xs <?php echo $current_page == 'page_messenger_bot.php' ? 'text-indigo-400 font-bold' : 'text-gray-500 hover:text-gray-300'; ?>"><?php echo __('auto_reply_messages'); ?></a>
                </div>
            </div>
            ...
            <div>
                <button @click="sidebarCollapsed ? (toggleSidebar(), waOpen = true) : waOpen = !waOpen" 
                    class="w-full flex items-center px-3 py-2.5 rounded-xl text-gray-400 hover:bg-white/5 hover:text-white transition-all group">
                    <div class="w-6 h-6 flex items-center justify-center shrink-0 transition-all duration-300" :class="sidebarCollapsed ? 'w-full' : 'w-6'">
                        <svg class="w-5 h-5 group-hover:text-green-400 transition-colors" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="ms-3 text-sm font-medium hide-on-collapse transition-all duration-300"><?php echo __('whatsapp'); ?></span>
                    <svg class="w-4 h-4 ms-auto transition-transform hide-on-collapse" :class="waOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    </svg>
                </button>
                <div :class="{ 'is-active': waOpen && !sidebarCollapsed }"
                    ...
                    <a href="wa_accounts.php" title="<?php echo __('wa_accounts'); ?>" class="block py-1.5 text-xs <?php echo $current_page == 'wa_accounts.php' ? 'text-indigo-400 font-bold' : 'text-gray-500 hover:text-gray-300'; ?>"><?php echo __('wa_accounts'); ?></a>
                    <a href="wa_bulk_send.php" title="<?php echo __('wa_bulk_send'); ?>" class="block py-1.5 text-xs <?php echo $current_page == 'wa_bulk_send.php' ? 'text-indigo-400 font-bold' : 'text-gray-500 hover:text-gray-300'; ?>"><?php echo __('wa_bulk_send'); ?></a>
                    <a href="wa_settings.php" title="<?php echo __('wa_settings'); ?>" class="block py-1.5 text-xs <?php echo $current_page == 'wa_settings.php' ? 'text-indigo-400 font-bold' : 'text-gray-500 hover:text-gray-300'; ?>"><?php echo __('wa_settings'); ?></a>
                </div>
            </div>
