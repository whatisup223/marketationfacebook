                    ...
                    <div
                        ...
                        <div class="p-4 bg-[#18191a] h-[480px] flex flex-col font-sans">
                            ...
                            <div class="flex-1 overflow-y-auto pr-1 messenger-scrollbar space-y-4">
                                ...
                                <!-- Summary Hint -->
                                <div
                                    class="bg-pink-600/5 rounded-2xl border border-pink-500/10 p-4 text-center mt-4">
    ...
    function autoModerator() {
        return {
            ...
            async fetchWebhookInfo() {
                try {
                    const res = await fetch('ajax_moderator.php?platform=instagram&action=get_webhook_info');
            ...
            async fetchTokenDebug() {
                ...
                try {
                    const res = await fetch(`ajax_moderator.php?platform=instagram&action=get_token_debug&page_id=${this.selectedPageId}`);
            ...
            async subscribePage() {
                ...
                try {
                    ...
                    const res = await fetch('ajax_moderator.php?platform=instagram&action=subscribe_page', {
                        method: 'POST',
                        body: formData
                    });
                    const data = await res.json();
            ...
            async stopProtection() {
                ...
                try {
                    ...
                    formData.append('page_id', this.selectedPageId);
                    formData.append('platform', 'instagram');
                    const res = await fetch('ajax_moderator.php?platform=instagram&action=unsubscribe_page', {
                        method: 'POST',
                        body: formData
                    });
                    const data = await res.json();
            ...
            async loadRules() {
                if (!this.selectedPageId) return;
                try {
                    const res = await fetch('ajax_moderator.php?platform=instagram&action=get_rules&page_id=' + this.selectedPageId);
                    const result = await res.json();
                    if (result.status === 'success') {
                        this.rules = {
            ...
            async saveRules() {
                ...
                try {
                    ...
                    const res = await fetch('ajax_moderator.php?platform=instagram&action=save_rules', {
                        method: 'POST',
                        body: formData
                    });
                    const result = await res.json();
            ...
            async loadLogs() {
                ...
                try {
                    const res = await fetch('ajax_moderator.php?platform=instagram&action=get_logs' + (this.selectedPageId ? '&page_id=' + this.selectedPageId : ''));
                    const result = await res.json();
                    if (result.status === 'success') {
            ...
            async deleteLog() {
                ...
                try {
                    ...
                    const res = await fetch('ajax_moderator.php?platform=instagram&action=delete_log', {
                        method: 'POST',
                        body: formData
                    });
                    const data = await res.json();
